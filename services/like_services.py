...
def enviar_like(user_id,post_id,db):
    ...
    verificar_se_curtiu = db.query(Like).filter(Like.global_chat_id == post_id.post_id, Like.user_id == user_id).first()

    if not verificar_se_curtiu:

        new = Like(
            user_id=user_id,
            global_chat_id=post_id.post_id
        )

        db.add(new)
        db.commit()

        total_likes = db.query(Like).filter(Like.global_chat_id == post_id.post_id).count()
